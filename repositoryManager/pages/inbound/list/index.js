...
Page({
  ...
  onShow() {
    // 加载入库记录
    const inboundRecords = storage.getData('inboundRecords').sort((a, b) => {
    ...
    this.setData({
      inboundRecords,
      inboundTypes,
      filteredRecords: inboundRecords
    });
  ...
  // 应用所有筛选条件
  applyFilters() {
    const { inboundRecords, searchKey, filter } = this.data;
  ...
  viewDetail(e) {
    ...
    wx.navigateTo({
      url: `/pages/inbound/detail/index?id=${id}`
    });
  },
